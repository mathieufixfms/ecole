...
class StudentDao(Dao[Student]):
	...
	def update(self, student: Student) -> bool:
		"""Met à jour l'enregistrement student (met à jour id_person si présent)."""

		id_person = getattr(student, 'id_person', None) or getattr(student, 'id', None)
		student_nbr = getattr(student, 'student_nbr', None)

		with Dao.connection.cursor() as cursor:
			sql = "UPDATE student SET id_person = %s WHERE student_nbr = %s"

			cursor.execute(sql, (id_person, student_nbr))
	...
	def create(self, student: Student) -> int:
		...
		try:
			with Dao.connection.cursor() as cursor:
				# Le compteur en mémoire est réinitialisé à chaque lancement.
				cursor.execute(
